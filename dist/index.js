...
function createEnergyCapacitor(config) {
    ...
    var trickleCharge = config.value(ENERGY_CAPACITOR_ID, 'trickleCharge');
    ...
    return {
        ...
        update: function update(prevState, input, globals) {
            var energy = clamp(input.storedEnergy + input.trickleChargeEnergy + input.energy, 0, prevState.capacity);
...
function createCore(config) {
    ...
    var capacitorTrickleCharge = config.value(ENERGY_CAPACITOR_ID, 'trickleCharge');
...
function createCore$1(config) {
    ...
    var drinkingWaterRequiredSilentRunning = config.value(BASE_ID, 'silentRunningDrinkingWaterRequired') / HOUR_TO_TICK$2;
    var drinkingWaterRequiredLockdown = config.value(BASE_ID, 'lockdownDrinkingWaterRequired') / HOUR_TO_TICK$2;
    ...
    return {
        ...
        update: function update(prevState, input, globals) {
            ...
            var requiredDrinkingWater = globals.lockdown ? drinkingWaterRequiredLockdown : globals.silentRunning ? drinkingWaterRequiredSilentRunning : drinkingWaterRequired;
            return {
